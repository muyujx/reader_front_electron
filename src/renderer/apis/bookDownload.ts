...
export function downloadBook(bookInfo: BookInfo): Promise<{ success: boolean, bookId: number }> {
    return ipcInvoke(ipcChannel.bookDownload, bookInfo);
}
 ...
 * 获取书籍下载进度
 */
export function getDownloadProgress(bookId: number): Promise<{ exists: boolean, downloadedPages: number, totalPage: number }> {
...
export function getLocalBookInfo(bookId: number): Promise<any> {
